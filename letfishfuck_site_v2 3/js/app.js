...
// LFF build id (debug)
const LFF_BUILD = 'v17-stable';

  // Namespace
  const LFF = window.LFF = window.LFF || {};
  LFF.BUILD = LFF_BUILD;
  LFF.STARTED_AT = Date.now();
...
async function initByPage() {
    ...
    // Pick the first selector that exists. Keeps older HTML ids from rendering blank.
    const pick = (...sels) => {
    ...
    if (page === 'work') {
      const container = pick('#projectsList', '#projectsGrid');
      if (!container) return;
      await renderProjects({container}); // work list uses projects
      return;
    }
    if (page === 'media') {
      const container = pick('#mediaList', '#mediaGrid', '#projectsGrid');
      if (!container) return;
      await renderProjects({container, filterKind:'media'});
      return;
    }
    ...
    if (page === 'notes') {
      const container = pick('#notesList', '#notesGrid', '#projectsGrid');
      if (!container) return;
